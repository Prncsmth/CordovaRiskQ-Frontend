...
**Post-implementation correction (found via live device testing, not caught by tsc/eslint/the web bundle check):** the `name` below is wrong — it must be `"change-password/index"`, the full route name Expo Router registers for a `folder/index.tsx` file, not the bare folder name. With the name as originally written, Expo Router's screen matcher misses, logs a console warning, and silently drops the `options` — so `transparentModal` never applied and Profile rendered hidden instead of dimmed. `animation` should also be `"fade"`, not `"slide_from_bottom"` — the latter moves the entire transparent screen including the backdrop, making the dim sweep up instead of fading in place. The code block below is left as originally planned/executed for the historical record; the actual shipped code (commit `d5b5ad9`) has both corrections.
